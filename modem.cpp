...
void merge(int i, int mid, int j){
    ...
    left = vector<int>(j - mid);
    right = vector<int>(mid - i + 1);

    for(int k = 0; k < left.size(); k++){
        left[k] = listOfEdges[i + k];
    }

    for(int k = 0; k < right.size(); k++){
        right[k] = listOfEdges[mid + j + k];
    }

    int indexLeft = 0, indexRight = 0;
    while(indexLeft < j - mid && indexRight < mid - i + 1){
        if(left[indexLeft] < right[indexRight]){
            listOfEdges[i] = left[indexLeft];
            indexLeft++;
        } else {
            listOfEdges[i] = right[indexRight];
            indexRight++;
        }
        i++;
    }

    while(indexLeft < j - mid){
        listOfEdges[i] = left[indexLeft];
        indexLeft++;
        i++;
    }

    while(indexRight < mid - i + 1){
        listOfEdges[i] = right[indexRight];
        indexRight++;
        i++;
    }
